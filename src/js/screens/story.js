...
export async function storyScreen(id) {
  try {
    ...
    const cover=story.imagen?`<img class="story-featured-cover" src="./${escapeHtml(story.imagen)}" alt="Ilustración de ${escapeHtml(story.titulo)}" loading="eager">`:'';
    return `<div class="story-shell"><a class="story-back" href="#lectura?tipo=stories&edad=${story.edad_min}-${story.edad_max}">← Volver a cuentos</a><p class="eyebrow">Cuento · ${escapeHtml(story.nivel_lector)}</p><h1>${escapeHtml(story.titulo)}</h1><div class="mission-meta"><span class="pill">${escapeHtml(story.edad_min)}–${escapeHtml(story.edad_max)} años</span><span class="pill warm">💛 ${escapeHtml(story.valor)}</span>${completed ? '<span class="pill">✓ Leído</span>' : ''}</div>${cover}${keywordIntro(story.palabra_clave)}<section class="story-pages" aria-label="Páginas del cuento">${story.paginas.map((page, index) => `<article class="card story-page"><span class="story-page-number">${index + 1}</span><p>${highlightKeyword(page, story.palabra_clave.palabra)}</p></article>`).join('')}</section>${keywordPractice(story.palabra_clave)}<section class="section card story-quiz"><p class="eyebrow">Reto de comprensión</p><h2>¿Qué recuerdas del cuento?</h2><p class="muted">Puedes volver a leer cualquier página. No hay prisa.</p><form id="story-quiz-form" data-story-id="${escapeHtml(story.id)}">${story.preguntas.map((question, questionIndex) => `<fieldset data-answer="${Number(question.correcta)}"><legend>${questionIndex + 1}. ${escapeHtml(question.enunciado)}</legend>${question.opciones.map((option, optionIndex) => `<label class="story-option"><input type="radio" name="question-${questionIndex}" value="${optionIndex}" required><span>${escapeHtml(option)}</span></label>`).join('')}</fieldset>`).join('')}<button class="button" type="submit">Comprobar respuestas</button></form><div class="feedback" id="story-feedback" role="status" aria-live="polite" hidden></div></section></div>`;
  } catch (error) {
