...
const routes = {inicio:homeScreen, bienvenida:onboardingScreen, mini:miniScreen, misiones:missionsScreen, lectura:readingScreen, logros:achievementsScreen, valores:valuesScreen, seguridad:securityScreen, adultos:adultsScreen};
...
async function render() {
  ...
  const view = route === 'jugar' ? () => gameScreen(id) : route === 'reto-seguro' ? () => securityGameScreen(id) : route === 'cuento' ? () => storyScreen(id) : route === 'mini-juego' ? () => miniGameScreen(id) : route === 'mini-cuento' ? () => miniStoryScreen(id) : route === 'mini-sonido' ? miniSoundScreen : routes[route] || homeScreen;
  closeMobileMenu();
